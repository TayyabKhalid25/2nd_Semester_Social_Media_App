...
SocialMediaApp::SocialMediaApp(QWidget *parent) : QMainWindow(parent), ui(new Ui::SocialMediaApp)
{
    ui->setupUi(this);
    currentUser = 0;
    totalUsers = totalPages = totalPosts = totalMemories = 0;
    users = 0;
    pages = 0;
    posts = 0;
    memories = 0;
...
void SocialMediaApp::loadData()
{
    ...
    if (database.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        while (!fin.atEnd()) {
            fin >> totalUsers >> totalPages >> totalPosts >> totalMemories;
            users = new User*[totalUsers] {0};
            pages = new Page*[totalPages] {0};
            posts = new Post*[totalPosts] {0};
            memories = new Memory*[totalMemories] {0};
...
void SocialMediaApp::saveData()  // Incomplete
{
    QFile database("Database.txt");
...
void SocialMediaApp::setCurrentUser()  // Load User and view user's home, timeline, friends.
{
    bool ok;
    int userID = QInputDialog::getInt(this, "Enter UserID ", "Number:", 0, 1, totalUsers + 1, 1, &ok);
    if (ok) {
    ...
    if (ok) {
        if (postID < totalPosts + 1) {  // Check for valid ID.
            ...
            if (ok && !text.isEmpty()) {
                ...
                Memory* memory = new Memory(post, totalMemories + 1, currentUser, text, Activity(0, QString("")), 0, 0, QDate::currentDate());
                memories[totalMemories++] = memory;
            }
        }
    }
}
